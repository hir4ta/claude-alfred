...
`alfred doctor` でセットアップの健全性を一目で確認。

**チェック項目 (8つ):**
1. Bun >= 1.3
2. 13 hooks 全登録 (`~/.claude/settings.json`)
3. `/alfred:review` skill 存在 (`~/.claude/skills/alfred-review/SKILL.md`)
4. `alfred-reviewer` agent 存在 (`~/.claude/agents/alfred-reviewer.md`)
5. `alfred-quality` rules 存在 (`~/.claude/rules/alfred-quality.md`)
6. `gates.json` 存在 + on_write に1つ以上のgate
7. `.alfred/.state/` 存在
8. `alfred` が PATH に存在

**出力:** `[OK]` / `[FAIL]` / `[WARN]` + 修正方法のヒント。

**実装:** `src/doctor.ts` (現在 stub) を実装。テスト: doctor がチェック項目を正しく判定するか。
