...
Used by `/alfred:attend` when things go wrong.
...
2. After resolving: run `/alfred:attend {task-slug}` to resume
...
When `/alfred:attend {slug}` is called and state shows `blocked: true`:
