...
- dossier validate: read-only progressive structural checker — 12 checks: required_sections, min_fr_count (S:1+, M:3+, L:5+), traceability (fr_to_task, task_to_fr), confidence_annotations, closing_wave, design_fr_references, testspec_fr_references, nfr_traceability (L/XL only), gherkin_syntax (Given+When+Then required), orphan_tests (source annotation validation), orphan_tasks (Requirements line validation); returns JSON report with checks array
