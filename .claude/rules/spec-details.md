...
- Task status: 6-state model (pending/in-progress/review/done/deferred/cancelled) with VALID_TRANSITIONS state machine

## Spec Review & Approval Gate (from CLAUDE.md)
...
- Approval gate (M/L/XL): dossier complete checks BOTH _active.md review_status AND verifyReviewFile()
...
- Audit log: .alfred/audit.jsonl (spec.init, spec.delete, spec.complete, review.submit, task.status_change)
