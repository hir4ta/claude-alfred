...
--- Metrics (293 actions across 5 sessions) ---
  ...
  False Positives:
    Avg DENY resolution time: 45s
    Pace-red FP rate: 25%
    LOC-limit FP rate: 10%
...
### False Positives セクション

不要な DENY を検出し、自動キャリブレーションにフィードバック:

| 指標 | 意味 |
|------|------|
| **Avg DENY resolution time** | pace-red/loc-limit DENY からコミットまでの平均秒数 |
| **Pace-red FP rate** | pace-red DENY 後に全 gate 通過でコミットできた割合。高いほど閾値が厳しすぎる |
| **LOC-limit FP rate** | loc-limit DENY 後に全 gate 通過でコミットできた割合。高いほど制限が厳しすぎる |

FP 率が 20% を超えると calibration が自動で pace_files (+1-5) / loc_limit (+10-50) を緩和する。
...
| 閾値 | デフォルト | 範囲 | 調整基準 |
|------|-----------|------|----------|
| **pace_files** | 15 | 10-30 | first-pass rate (40-90%) で補間。pace-red FP rate >20% で +1-5 加算 |
| **review_file_threshold** | 5 | 3-7 | review-miss rate (0-10%) で補間 |
| **context_budget** | 2000 | 1500-2500 | respond-skipped rate (0-30%) で補間 |
| **loc_limit** | 200 | 150-400 | avg fix effort (1-4 edits) で補間。loc-limit FP rate >20% で +10-50 加算 |
...
`/qult:plan-generator` で 1 文の機能説明から構造化 Plan を自動生成:

```
/qult:plan-generator "JWT認証をAPIに追加"
  → qult-plan-generator (Opus) が codebase を分析
  → WHAT/WHERE/VERIFY/BOUNDARY/SIZE 形式の Plan を生成
  → 4+ tasks なら /qult:plan-review で自動検証
  → .claude/plans/plan-YYYYMMDD-HHMMSS.md に書き出し
```

Anthropic の [Harness Design](https://www.anthropic.com/engineering/harness-design-long-running-apps) 記事が提示した 3-agent パターンを完成させる:

| 役割 | エージェント | 機能 |
|------|------------|------|
| **Planner** | `/qult:plan-generator` + `qult-plan-generator` | 1文 → full plan 自動生成 |
| **Generator** | Claude 本体 | Plan に従ってコーディング |
| **Evaluator** | `/qult:review` + `qult-reviewer` | 独立レビュー + スコア閾値強制 |
