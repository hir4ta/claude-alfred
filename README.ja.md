...
│  Stop / SubagentStop → LLM Decision 抽出     │
...
| Hook | タイミング | 動作 |
|------|-----------|------|
| `SessionStart` | セッション開始時 | プロジェクト記録 + CLAUDE.md 自動取り込み + コンパクション後のコンテキスト再注入 |
...
| `SubagentStart` | subagent 起動時 | コンパクトコンテキスト（過去 decisions + 最近のファイル）を注入 |
| `Stop` | アシスタント応答終了時 | LLM で応答から Decision を抽出（非同期） |
| `SubagentStop` | subagent 終了時 | LLM で subagent 応答から Decision を抽出（非同期） |
...
**キーバインド:** `↑↓` 移動、`Enter` 展開/折りたたみ、`g/G` 先頭/末尾、
`1`/`2` または `Tab` でタブ切替（Activity / Decisions）、`?` ヘルプ、`q` 終了
