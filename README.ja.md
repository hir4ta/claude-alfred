...
**積み重なる記憶。** 「あの時こう決めた」「このバグ前にも見た」「Xは試したけどダメだった」— 全てがvitalityスコア付きセマンティックメモリに保存。仮定は速く腐り、実証されたルールは長く残る。矛盾は自動検出。次に似た問題に当たったとき、聞く前に alfred が出してくる。
...
**バイパスできない承認ゲート。** 実装前に仕様のレビューサイクルを回す。TUI ダッシュボードで任意の行にコメントして、承認か差し戻し — GitHub の PR レビューと同じ体験を、仕様に対してやる。ステータスの手動書き換えでは突破できない。レビューファイルの存在まで検証する。
...
| `ledger` | メモリ — search, save（有効期限付き）, promote, reflect, stale, audit-conventions |
...
| PreCompact | 決定抽出 → 構造化チャプターメモリ (JSON) 保存 → エピック進捗同期 → 調査パターン検出・保存リマインド |
...
| **Tasks** | タスクカード: 現在のフォーカス、次のアクション、エピック、バリデーションバッジ。ドリルダウンで仕様ファイルの中身をプレビュー |
| **Knowledge** | メモリ一覧（サブタイプ別タグ付き）。`Ctrl+F` でローカルフィルタ、`Ctrl+S` で Voyage AI セマンティック検索（関連度スコア付き）。`Space` でメモリの有効/無効を切り替え |
| **Activity** | 操作タイムライン。`f` でフィルタ切替（init/complete/review）。エピックからタスクへ直接ジャンプ |

インラインレビュー: 仕様ファイルで `r` を押すと行番号付きビューに。特定の行にコメントして、レビューラウンドを切り替えて、承認または差し戻し — ターミナルから出ずに完結する。
...
メモリは静的じゃない。ライフサイクルがある:

- **サブタイプ減衰**: 仮定は30日で腐る。実証されたルールは120日持つ。メモリの種類ごとに半減期が違う。
- **Vitalityスコア**: 0-100の複合スコア（鮮度、参照回数、タイプ重み、アクセス頻度）。低スコアのメモリはフラグされるが、勝手に消されることはない。
- **矛盾検出**: 2つのメモリが矛盾していたら（「JWT使え」vs「JWT避けろ」）、自動でフラグ。
- **有効期限**: 変化の速いAPI決定に明示的な期限を設定。期限切れメモリは検索から消える。
- **メモリバージョニング**: メモリを更新しても旧版は保持。チェーン最大5バージョン。
...
```bash
alfred steering-init  # go.mod, README, CLAUDE.md から自動生成
```
...
ストレージ
  ...
  |-- .alfred/audit.jsonl  -> 操作監査ログ + ドリフトイベント
  +-- ~/.claude-alfred/    -> SQLite（records + FTS5 + embeddings, スキーマ V7）
```
...
| `.alfred/steering/` | `alfred steering-init` 実行時 | CLI コマンド（プロジェクト解析） |
