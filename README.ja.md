...
**git でチーム共有。** ナレッジは構造化 JSON — コミットして PR でレビューしてチームで共有。`alfred team init` でカスタム merge driver を設定し、ナレッジの衝突を構造的に解決。`team.yaml` でマルチレビュアー承認、spec の git 管理、横断検索を設定。サーバー不要 — git がトランスポート。
