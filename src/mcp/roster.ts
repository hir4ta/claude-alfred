...
export async function handleRoster(_store: Store, params: RosterParams) {
	...
	switch (params.action) {
		case "init":
			return rosterInit(projectPath, params);
		case "status":
			return rosterStatus(projectPath, params);
		case "link":
			return rosterLink(projectPath, params);
		case "unlink":
			return rosterUnlink(projectPath, params);
		case "order":
			return rosterOrder(projectPath, params);
		case "list":
			return rosterList(projectPath);
		case "update":
			return rosterUpdate(projectPath, params);
		case "delete":
			return rosterDelete(projectPath, params);
		default:
	...
	if (!params.confirm) {
		...
		try {
			...
			return jsonResult({
				...
				warning:
					"Tasks (specs) will NOT be deleted — they become standalone. Pass confirm=true to proceed.",
			});
		} catch (err) {
			return errorResult(`${err}`);
		}
