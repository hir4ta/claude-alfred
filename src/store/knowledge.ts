...
export function upsertKnowledge(store: Store, row: KnowledgeRow): UpsertResult {
  ...
  ).get(row.projectRemote, row.projectPath, row.filePath) as { id: number; content_hash: string } | undefined;
...
export function deleteKnowledgeByProject(store: Store, projectRemote: string, projectPath: string): number {
  const txn = store.db.transaction(() => {
    store.db.prepare(`
...
export function listKnowledge(store: Store, projectRemote: string, projectPath: string, limit: number): KnowledgeRow[] {
  const rows = store.db.prepare(`
    SELECT id, file_path, content_hash, title, content, sub_type,
           project_remote, project_path, project_name, branch,
           created_at, updated_at, hit_count, last_accessed, enabled
...
export function listAllKnowledge(store: Store, projectRemote: string, projectPath: string, limit: number): KnowledgeRow[] {
  const rows = store.db.prepare(`
    SELECT id, file_path, content_hash, title, content, sub_type,
           project_remote, project_path, project_name, branch,
           created_at, updated_at, hit_count, last_accessed, enabled
...
export function getKnowledgeStats(store: Store): KnowledgeStats {
  ...
  const subtypeRows = store.db.prepare(
