...
import type { KnowledgeRow, KnowledgeConflict } from '../types.js';
import { searchKnowledgeKeyword, getKnowledgeByIDs, mapRow } from './knowledge.js';
...
export function expandAliases(store: Store, terms: string[]): string[] {
  ...
  for (const t of terms) {
    ...
    const aliasRows = store.db.prepare(
      'SELECT alias FROM tag_aliases WHERE LOWER(tag) = ?',
    ).all(lower) as Array<{ alias: string }>;
    for (const r of aliasRows) seen.add(r.alias.toLowerCase());

    const tagRows = store.db.prepare(
      'SELECT tag FROM tag_aliases WHERE LOWER(alias) = ?',
...
function classifyConflict(contentA: string, contentB: string): 'potential_duplicate' | 'potential_contradiction' {
