import type { SessionContinuity, SessionLink } from "../types.js";
...
export function resolveMasterSession(store: Store, claudeSessionId: string): string {
	...
	for (;;) {
		...
		seen.add(id);
		const row = store.db
			.prepare("SELECT master_session_id FROM session_links WHERE claude_session_id = ?")
