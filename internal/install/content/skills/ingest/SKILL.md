...
# /alfred:ingest — Reference Material Processor
...
1. Call `spec` with action=update, file=requirements.md, mode=append
2. Append the structured summary under a `## Reference Materials` section

**If no spec but task-oriented:**
1. Call `recall` with action=save
2. Save as permanent memory with label="Ingested: <topic>"

**Always:**
- Confirm what was saved and where
- Suggest next step: "/alfred:plan to create a spec from this" or "describe what you want to build"
